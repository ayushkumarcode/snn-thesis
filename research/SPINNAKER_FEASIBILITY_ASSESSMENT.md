...
1. Rhodes, O. et al. (2018). "sPyNNaker: A Software Package for Running PyNN Simulations on SpiNNaker." *Frontiers in Neuroscience*.
2. Pedersen, J. et al. (2024). "Neuromorphic intermediate representation: A unified instruction set for interoperable brain-inspired computing." *Nature Communications*.
3. Arfa, S. et al. (2025). "Efficient Deployment of Spiking Neural Networks on SpiNNaker2 for DVS Gesture Recognition Using NIR." *arXiv:2504.06748*.
4. Arfa, S. et al. (2025). "Hardware-Aware Fine-Tuning of Spiking Q-Networks on the SpiNNaker2 Neuromorphic Platform." *arXiv:2507.23562*.
5. Dominguez-Morales, J.P. et al. (2016). "Multilayer Spiking Neural Network for Audio Samples Classification Using SpiNNaker." *Springer*.
6. Ezinwoke, B. & Rhodes, O. (2025). "Predicting Price Movements in High-Frequency Financial Data with Spiking Neural Networks." *arXiv:2512.05868*.
...
| Finding | Confidence |
|---------|-----------|
| NIR conversion pathway exists snnTorch -> SpiNNaker2 | HIGH (demonstrated in papers) |
| SpiNNaker1 accessible via EBRAINS for free | HIGH (official EBRAINS documentation) |
| SpiNNaker2 NOT publicly accessible to students | HIGH (private repos, restricted access) |
| sPyNNaker supports LIF neurons | HIGH (core functionality) |
| Convolutional support on SpiNNaker1 exists but is non-trivial | HIGH (KernelConnector documented) |
| Weight precision loss will occur (32-bit -> 16-bit or 8-bit) | HIGH (fundamental constraint) |
| Time estimate of 2-4 weeks for SpiNNaker addition | MODERATE (depends on architecture complexity) |
| Previous Manchester undergrads likely did NOT use SpiNNaker | MODERATE (inferred from available evidence) |
| Oliver Rhodes can facilitate SpiNNaker access | MODERATE (inferred from his role) |
| Robot reflexes project is best fit for SpiNNaker | HIGH (direct precedent with spiking Q-networks) |
