...
The continual learning experiment (§6.2) confirms the expected finding — both SNN and ANN suffer severe catastrophic forgetting — but provides a quantitative comparison and an unexpected finding: **the SNN forgets 6.9 pp less than the ANN** (74.4% vs 81.3% mean forgetting).

**Why the SNN forgets less:** The mechanism is the same as for adversarial robustness — the spike threshold acts as a computational gate. During backward pass, only weights connected to neurons that actually fired receive gradient updates. With 74.16% activation sparsity, approximately 74% of weights receive zero gradient per forward pass. When fine-tuning on a new task, fewer weights are modified, preserving more of the earlier task representation. The ANN's dense activations produce non-zero gradients for all weights on all forward passes, enabling more complete overwriting.

**Why both forget aggressively:** The experimental setting is extreme — no replay, no regularisation, 320 training samples per task out of 1,600 total. The gradient pressure on each task is proportionally large relative to the task size. Mean forgetting of 74.4% and 81.3% significantly exceeds the 30–50% typical in regularisation-equipped continual learning. This is baseline forgetting, not regularised forgetting, and should be interpreted as such.
...
**Implications for future work:** The baseline forgetting rate (74.4% for SNN) quantifies the improvement achievable by SNN-specific consolidation mechanisms. Golden et al. (2022, PLoS Comp Bio) demonstrated sleep-based consolidation reducing forgetting by >50% in simpler spike-rate networks. Applied to this architecture, that would project to ~37% forgetting with consolidation, approaching the 30–40% ANN forgetting with EWC regularisation (Kirkpatrick et al. 2017).

**Limitation:** The experiment uses a single fold (fold 4), 20 epochs per task, and no hyperparameter tuning specific to the continual setting (lr=5e-4 was chosen to be smaller than training lr=1e-3 to reduce catastrophic overwriting). Results are indicative, not definitive.
