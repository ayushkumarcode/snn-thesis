...
Using NeuroBench v2.2.0 (Yik et al. 2025, Nature Communications), SynapticOperations metrics are computed for all SNN variants and the ANN baseline: the direct SNN uses 1.08M accumulate-only operations per sample (976 nJ estimated), versus 101K multiply-accumulate operations for the ANN (463 nJ). In software simulation, the SNN is 2.1× more expensive due to T=25 timestep overhead. On dedicated neuromorphic hardware, each AC costs 5.1× less than a MAC — closing this gap. The PANNs + SpiNNaker FC₂ deployment is the Pareto-optimal operating point: 92.50% accuracy, ~86 nJ for the SpiNNaker classification step.
